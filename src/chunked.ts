...
const calcChunkIndex = (index: number, chunkSize: number) =>
  Math.floor(index / chunkSize);
const calcIndexInChunk = (index: number, chunkSize: number) =>
  index % chunkSize;

/**
 * ChunkedBinaryIndexedTree implementation
 */
export default class ChunkedBinaryIndexedTree
  implements BinaryIndexedTreeInstance
{
  private _chunks: BinaryIndexedTree[] = [];

  constructor(private _chunkSize: number) {}

  /**
   * @param {Array<number>} seed - BIT will be built from this array
   * @returns {ChunkedBinaryIndexedTree} instance
   * O(N)
   */
  static build(seed: number[], chunkSize: number): ChunkedBinaryIndexedTree {
    const ret = new ChunkedBinaryIndexedTree(chunkSize);
    let chunks = calcChunkIndex(seed.length - 1, chunkSize) + 1;
    ret._chunks.length = chunks;
    for (let i = 0; i < chunks; i++) {
      ret._chunks[i] = BinaryIndexedTree.build(
        seed.slice(0, chunkSize * (i + 1))
      );
    }
    return ret;
  }
  ...
  get length(): number {
    let ans = 0;
    for (const chunk of this._chunks) ans += chunk.length;
    return ans;
  }
   ...
   * O(N * log(N))
   */
  find(check: Checker<ChunkedBinaryIndexedTree>): number | undefined {
